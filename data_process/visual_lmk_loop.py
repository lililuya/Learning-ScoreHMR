...
def match_directories(image_dir, dense_lmk_dir, lmk_dir):
    lmk_files = os.listdir(lmk_dir)
    dense_lmk_files = os.listdir(dense_lmk_dir)
    image_files = os.listdir(image_dir)
    
    # 提取文件名的共同部分（不含扩展名）
    lmk_filenames = {os.path.splitext(filename)[0] for filename in lmk_files}
    dense_lmk_filenames = {os.path.splitext(filename)[0] for filename in dense_lmk_files}
    image_filenames = {os.path.splitext(filename)[0] for filename in image_files}
    
    # 找出三个目录中共同存在的文件名
    common_filenames = lmk_filenames & dense_lmk_filenames & image_filenames
    
    # 构建匹配后的路径组
    matched_paths = []
    for filename in common_filenames:
        lmk_path = os.path.join(lmk_dir, filename + '.npy')
        dense_lmk_path = os.path.join(dense_lmk_dir, filename + '.npy')
        image_path = os.path.join(image_dir, filename + '.png')
        matched_paths.append((image_path, dense_lmk_path, lmk_path))
...
if __name__=="__main__":
    ...
    dict_pair = match_directories(image_dir, dens_ldm_dir, ldm_dir)
    for pair in sorted(dict_pair):
        # gt
        basename = os.path.basename(os.path.splitext(pair[0])[0])
        ...
        # dense
        dense_ldm_arr = np.load(pair[1], allow_pickle=True)
        ...
        # 68
        landmarks68 = np.load(pair[2], allow_pickle=True)
        ...
        cv2.imwrite('{}/{}.png'.format(savefolder, basename), final_views)
